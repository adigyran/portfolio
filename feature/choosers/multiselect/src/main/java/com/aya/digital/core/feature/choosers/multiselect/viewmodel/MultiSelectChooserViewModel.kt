...
class MultiSelectChooserViewModel(
    private val coordinatorRouter: CoordinatorRouter,
    private val getMultiSelectItemsUseCase: GetMultiSelectItemsUseCase,
    private val param: MultiSelectChooserView.Param
) :
    BaseViewModel<MultiSelectChooserState, BaseSideEffect>() {
    override val container = container<MultiSelectChooserState, BaseSideEffect>(
        initialState = MultiSelectChooserState(),
    )
    {
        if (it.items.isEmpty()) {
            loadItems("")
    ...
    fun applyFilters() = intent {
        coordinatorRouter.sendEvent(MultiSelectChooserNavigationEvents.FinishWithResult(param.requestCode,
