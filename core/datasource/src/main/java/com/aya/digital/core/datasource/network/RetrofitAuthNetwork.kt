...
fun authNetworkModule() = DI.Module("authNetworkModule") {
    bind<AuthDataSource>() with singleton {
        val apiService =
            createApiService<AuthService>(instance())
...
class RetrofitAuthNetwork(private val network: AuthService) :
    AuthDataSource {
    override fun registerProfile(registrationBody: RegistrationBody): Completable = network.registerProfile(registrationBody)

    override fun generateToken(loginBody: LoginBody): Single<LoginResponse> = network.generateToken(loginBody)

    override fun verifyProfile(code: String): Completable  = network.verifyProfile(code)
